...
abstract class BaseCommentmetaManager extends AbstractBaseObjectManager {
	public function __construct(\framework\ApplicationContext $context) {
		parent::__construct($context);
		$this->setTableName('commentmeta');
	}
	...
	public function getCommentmeta($id) {
		$query = new \framework\query\QueryBuilder();
		$query->addSelect($this->getTableName().'.*');
		$query->addTable($this->getTableName());
		if(!is_array($id)) {
	...
	protected function getUpdateQuery(\model\Commentmeta $obj) {
	...
	protected function update(\model\Commentmeta $obj) {
		$query = $this->getUpdateQuery($obj);
		$this->database->execute($query->toSQL());
	}

	protected function getCreateQuery(\model\Commentmeta $obj) {
	...
	protected function create(\model\Commentmeta $obj) {

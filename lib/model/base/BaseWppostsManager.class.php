...
abstract class BaseWppostsManager extends AbstractBaseObjectManager {
	...
	/**
	 * Returns a limited and filter number of Wppostss	 *
	 * @param $start Number Index where we start to display Wppostss (by default 0)
	 * @param $maxperpage Number Maximum number of item per page
	 * @param $filter String null or a Wpposts filter
	 */
	public function getLimitedWppostss($start, $maxperpage, $filter) {
		...
		if($filter != null) {
			$f = new \model\WppostsFilter($filter);
			$f->apply($query);
		}
	...
	public function countLimitedWppostss($filter) {
		$query = new \framework\query\QueryBuilder();
	...
	protected function getUpdateQuery(\model\Wpposts $obj) {
	...
	protected function update(\model\Wpposts $obj) {
		$query = $this->getUpdateQuery($obj);
		return $this->database->execute($query->toSQL());
	}

	protected function getCreateQuery(\model\Wpposts $obj) {
	...
	protected function create(\model\Wpposts $obj) {
	...
	public function delete(\model\Wpposts $obj) {
